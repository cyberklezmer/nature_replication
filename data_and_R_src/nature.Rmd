...
load("../data_and_R_src/dataProcessed.RData")
selected = data
...
```{r}
# Dependent variable:
# reproducibility ... df$krize_rep
dfs = df %>% select(krize_rep, Author) %>% filter(!is.na(krize_rep)) %>%  
  group_by(Author) %>% mutate(N = n()) %>% 
  group_by(Author, krize_rep, N) %>% summarise(n = n()) %>% 
  mutate(f = round(n / N * 100, 1))
dfs.Jedlicka <- dfs %>% filter(Author=="Jedlička")
ggplot(dfs.Jedlicka, aes(y = fct_rev(krize_rep), x = f, fill="pink", label = paste(f, "%"))) +
  geom_col(alpha = 0.75, na.rm = T, position = position_dodge2(rev = T)) +
  geom_text(position = position_dodge2(width = 0.8, reverse = T)) +
  labs(title = "Is there reproducibility crisis?", x = "%", y="") +
  # scale_x_log10() +
  # guides(fill = "none") +
  theme_minimal()
df.Jedlicka <- df %>% filter(Author=="Jedlička")
# Explanatory variables:
# research area (obor) 
# sociodemographic factors
# stage of research career (41-48, ... ?)
# share of basic research (50)
# share of research (49)
```


```{r}
table(my[,c("krize_rep","spec_hlavni")])
table(my[,c("krize_rep","vek")])

```


2) Faktory reprodukovatelnosti 
- přejímáme baterii od Bakerové (ot. 16)
+ máme novou unikátní baterii faktorů (ot. 24 - z kval výzkumu, pozor: k této baterii máme i otázku č. 25 na "zavážnost")
Metoda: komparace ?.

3) Obor a Reprodukovatelnost v jednotlivých oborech (ot. 8) 
- předpokládáme vliv: Charakteristiky oboru (baterie v dotazníku se škálou Ot 51 (lze dichotomizovat), + alternativně další otázky v dotazníku: č. 54 "exaktnost oboru", č. 52 velikost oboru)
Metoda: comparison, cluster analysis (char. oborů), factor analysis?
Pozn: Ještě mě napadlo udělat nějakou kompozitní proměnnou z Otázek 18-20 (Pokoušel jsem se neúspěšně zopakovat svůjj experiment / někoho jiného / někdo nemohl reprodukovat můj), protože to je vlastně taky míra nereprodukovatelnosti v daném oboru

4) Geography 
- sledujeme vliv regionu (též může být v případě Baker zahrnut do regresního modelu)
+ naše otázky: odkud pocházejí vadné studie (ot 10 a 11)

5) Coping strategies
- jak reagujete na reprodukovatelnost (ot. 15)

6) něco už jsem probrali, ale posílám pro jistotu:

POZNÁMKY
1. Některé otázky z Nature nejsou v česku, a naopak
2. Respondenti – Nature I ČR  = měli by být jen ti, co dělají základní výzkum (zkontrolovat)
3. Cizinci v ČR (kval. odpovědi anglicky)
4. Některé nepovinné (tzn. Liší se N) – budeme vždy uvádet u konkrétní otázky (grafu)
5. +Hodnota Nevím (ČR) – srovnáváme srovnatelné N / ČR
6. Prac. zařazení (av, mu, jču, uk) – instituce mají různé hodnoty, ale lze uspořádat
7. Cut off score (do 12 minut na vyplnění- kontrola dat od Františka)
8. Kvalitativní otázky (některé zařadíme – např. země, jiné budou  jen výběrově pojednány v textu)
9. Anglické popisky – v základním souboru ne X ve výsledcích ano
10.Matematici -vyřešit
