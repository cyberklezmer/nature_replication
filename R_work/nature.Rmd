...
MŠ, přesněji, stejně jsme to spočítali z jejích dat. Nicméně bych neřekl, že jsme reprodkuovali její výzkum v Čechách. Ale když přiděláme CI, vidíme, že se ve dvou případech neprotínají. A to ani pokud Jedličku převážím tak, aby populace dle oborý byla stejná.
...
dfw <- dfjedlicka  %>% 
  left_join(dfbaker, by="spec_hlavni") %>% 
  mutate( w = Baker_N / N)
...
#  filter(krize_rep != "I don't know") %>%
  ...
  select(krize_rep, Author,spec_hlavni) %>%
  filter(Author == "Jedlička") %>%
  mutate(Author = "JedličkaNorm") %>%
  left_join(dfw, by="spec_hlavni") %>% 
  select(krize_rep, Author, w) %>%
  group_by(Author) %>%
  mutate(N = sum(w))  %>%
  group_by(Author, krize_rep, N) %>%
  summarise(n = sum(w), .groups = "drop") 
  
df_total <- bind_rows(dfraw,dfadj)
...
ggplot(df_total, aes(y = fct_rev(krize_rep), x = f, fill = Author, label = paste(f, "%"))) +
  ...
  labs(title = "Is there a reproducibility crisis?", x = "%", 
       caption = paste0("N_Baker = ", df_total[1, "N"], 
                        ", N_Jedlička = ", df_total[8, "N"])) +
  theme_minimal()
...
Pořadí obdélníčků je v kr_levels.

```{r replication test}
# Ensure the dataframe is ungrouped

kr_levels <- c("No crisis", "Yes, slight crisis",
    "Yes, significant crisis", "I don't know" )

reduced_dfs <- df_total %>%
  select(Author, krize_rep, n) %>%
  ungroup() %>%  
  mutate(
    krize_rep = factor(krize_rep, levels = kr_levels),
...
# Ensure krize_rep ordering is kept
data_df$krize_rep <- factor(data_df$krize_rep, levels = levels(reduced_dfs$krize_rep))

# Mosaic plot with your custom ordering for stacked values (krize_rep)
ggplot(data_df) +
...
Takže hypotéza *Náhodně vybraný vědecký týpek to má tak a tak* v Česku neplatí. Tak ted zkoumejme, zda to vyjde aspoň po zohlednění kontinentu, a rozdělíme Evropu na východní a západní. Klasifikace zemí je uložena v níže uvedeném souboru (Baker rozlišuje jen jednu Evropu).
...
#  filter(krize_rep != "I don't know") %>%
  ...
  mutate(se = sqrt((f * (100 - f)) / N),  # Standard error
         lower = f - 1.96 * se,           # 95% CI Lower Bound
         upper = f + 1.96 * se)  %>%
  mutate(Author_Spec = paste0(substr(spec_hlavni,1,3),Author))
...
ggplot(dfs, aes(y = fct_rev(spec_hlavni), x = mean, fill = Author, label = paste(round(mean,1), "%"))) +
  geom_col(alpha = 0.75, na.rm = TRUE, position = position_dodge(width = dodge_width), 
           width = 0.4) +  # Reduce bar width to avoid overlap
  geom_errorbar(aes(xmin = mean-se, xmax = mean+se), 
                ...
                position = position_dodge(width = dodge_width)) + 
  geom_text(position = position_dodge(width = dodge_width), hjust = -0.5) +
  labs(title = "How much work replicable", x = "%", 
       caption = paste0("N_Baker = ", df_total[1, "N"], 
                        ", N_Jedlička = ", df_total[8, "N"])) +
  theme_minimal()
